...
import { Search, User, LogOut, Moon, Sun, Calendar, Menu, X, Bell, ChevronDown, Settings, Home, Users, BarChart3, CreditCard, Star, MessageCircle, FileText } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ activeModule, onModuleChange }) => {
  ...
  const notifications = [
    { id: 1, title: 'New Professional Registration', message: 'Dr. Smith has registered as a new professional', time: '5 min ago', unread: true },
    { id: 2, title: 'System Alert', message: 'Server maintenance scheduled for tonight', time: '1 hour ago', unread: true },
    { id: 3, title: 'Revenue Milestone', message: 'Monthly revenue target achieved', time: '2 hours ago', unread: false },
  ];
  ...
  const getUserDisplayInfo = () => {
    if (!user) return { initials: 'A', name: 'Admin' };
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center">
            ...
            >
              <div className="relative">
                <Calendar className="h-12 w-12 text-red-600 dark:text-red-400 group-hover:scale-125 group-hover:rotate-12 transition-all duration-500" />
                <div className="absolute inset-0 bg-red-600/20 rounded-full scale-0 group-hover:scale-200 transition-transform duration-700 animate-pulse"></div>
              </div>
              <div>
            ...
            <div className="ml-10 flex items-center space-x-2">
              {navItems.map((item, index) => (
                <button
                  ...
                  className={`relative px-6 py-3 rounded-2xl text-sm font-semibold transition-all duration-500 hover:bg-white/60 dark:hover:bg-gray-800/60 hover:shadow-xl hover:scale-110 group animate-fade-in-up ${
                    activeModule === item.href
                      ? 'text-red-600 dark:text-red-400 bg-white/60 dark:bg-gray-800/60'
                ...
                >
                  ...
                  {item.name}
                  {activeModule === item.href && (
                    <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-red-600 to-pink-600 rounded-full animate-fade-in-up"></div>
                  )}
                  <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 h-1 bg-gradient-to-r from-red-600 to-pink-600 rounded-full w-0 group-hover:w-full transition-all duration-300 opacity-50"></div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <button className="hidden md:flex items-center space-x-2 px-6 py-3 bg-gray-100/80 dark:bg-gray-800/80 hover:bg-white dark:hover:bg-gray-700 rounded-2xl transition-all duration-500 hover:scale-110 hover:shadow-xl group backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50">
              <Search className="h-5 w-5 text-gray-600 dark:text-gray-400 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors group-hover:rotate-12 duration-300" />
              <span className="text-sm text-gray-600 dark:text-gray-400 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors">Search</span>
            </button>
            ...
            <div className="relative">
              ...
              >
                <Bell className="h-6 w-6 text-gray-600 dark:text-gray-300 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors group-hover:animate-bounce" />
                {unreadCount > 0 && (
                    ...
                    {notifications.map((notification) => (
                      <div
                        ...
                        className={`p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer ${
                          notification.unread ? 'bg-red-50 dark:bg-red-900/10' : ''
                        }`}
                      >
            ...
            <div className="relative">
              ...
              >
                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-red-600 to-pink-600 flex items-center justify-center ring-2 ring-white dark:ring-gray-700 group-hover:ring-red-400 transition-all duration-300">
                ...
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-600 animate-fade-in-up z-50 backdrop-blur-xl">
                  ...
                  <div className="py-2">
                    <button
                      ...
                      className="flex items-center px-4 py-3 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors w-full"
                    >
              ...
              {navItems.map((item) => (
                <button
                  ...
                  className={`flex items-center px-4 py-3 rounded-2xl transition-all duration-500 ${
                    activeModule === item.href
                      ? 'text-red-600 dark:text-red-400 bg-white/60 dark:bg-gray-800/60'
                      : 'text-gray-700 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400 hover:bg-white/60 dark:hover:bg-gray-800/60'
                  }`}
                >
